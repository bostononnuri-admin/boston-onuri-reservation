...
// 이모티콘 또는 이미지 경로(img:...)를 렌더링
function renderIcon(icon, size=22) {
  if (typeof icon === "string" && icon.startsWith("img:")) {
    return <img src={icon.slice(4)} alt="icon" style={{width:size,height:size,objectFit:"contain",verticalAlign:"middle"}}/>;
  }
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(120px,1fr))",gap:8}}>
              {SPACES.map(sp=>(
                <div key={sp.id} onClick={()=>{setSelSpace(sp);setView("book");}}
                  ...
                  <div style={{marginBottom:4,display:"flex",justifyContent:"center",alignItems:"center",height:28}}>{renderIcon(sp.icon,24)}</div>
                            ...
                            return (
                              <div key={r.id} style={{display:"grid",gridTemplateColumns:"auto 1fr auto",gap:12,alignItems:"center",borderRadius:c.radiusSm,padding:"10px 14px",background:c.chipBg,border:`1px solid ${c.primary}22`}}>
                                <div style={{textAlign:"center",minWidth:52}}>
                                  <div style={{display:"flex",justifyContent:"center",alignItems:"center",height:24}}>{renderIcon(sp?.icon||"📍",22)}</div>
        ...
        {!loading&&view==="book"&&selSpace&&!submitted&&(
          <div>
            ...
            <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:20,padding:"14px 16px",background:c.chipBg,borderRadius:c.radius,border:`1.5px solid ${c.primary}44`}}>
              <span style={{display:"flex",alignItems:"center"}}>{renderIcon(selSpace.icon,30)}</span>
              <div>
                          ...
                          <div style={{background:"#fff",border:`1.5px solid ${c.border}`,borderRadius:c.radiusSm,padding:14,marginBottom:14}}>
                            ...
                            <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:2}}>
                              ...
                              {Array(new Date(adminBookCalYear,adminBookCalMonth+1,0).getDate()).fill(null).map((_,i)=>{
                                const day=i+1, m=String(adminBookCalMonth+1).padStart(2,"0"), d=String(day).padStart(2,"0");
                                const ds=`${adminBookCalYear}-${m}-${d}`;
                                const dow=localDate(ds).getDay();
                                const isSel=adminBookDate===ds, isToday=ds===todayStr, isPast=ds<todayStr;
                                const abHol={...getHolidays(adminBookCalYear-1),...getHolidays(adminBookCalYear),...getHolidays(adminBookCalYear+1)};
                                ...
                                return (
                                  <button key={day} onClick={()=>setAdminBookDate(ds)} style={{
                                    ...
                                    border:isSel?`2px solid ${c.primary}`:isToday?`2px solid ${c.gold}`:"2px solid transparent",
                                    background:isSel?c.primary:"transparent",
                                    color:isSel?"#fff":abHol[ds]||dow===0?c.SUN:dow===6?c.SAT:c.text,
                                    fontSize:13,fontFamily:"inherit",cursor:"pointer",fontWeight:isToday?700:400,opacity:isPast?0.5:1,
                                  }}>
                                    <div>{day}</div>
                                    {abHol[ds]&&<div style={{fontSize:6,color:isSel?"rgba(255,255,255,0.8)":c.SUN,overflow:"hidden",whiteSpace:"nowrap"}}>{abHol[ds]}</div>}
                          ...
                          {adminBookDate&&(
                            <div style={{background:"#fff",border:`1.5px solid ${c.border}`,borderRadius:c.radiusSm,padding:14}}>
                              ...
                              <div style={{display:"flex",flexDirection:"column",gap:12}}>
                                ...
                                {adminBookIsWorshipBlocked&&<div style={{background:"#fff7ed",border:"1.5px solid #fb923c55",borderRadius:c.radiusSm,padding:"8px 12px",fontSize:12,color:"#c2410c"}}>⛪ 주일예배 시간({toAMPM(WORSHIP_START)}–{toAMPM(adminBookWorshipEnd)}) 예약 불가</div>}
                                {adminBookIsSatBlocked&&<div style={{background:"#fff7ed",border:"1.5px solid #fb923c55",borderRadius:c.radiusSm,padding:"8px 12px",fontSize:12,color:"#c2410c"}}>🎵 찬양 연습 시간({toAMPM(SAT_START)}–{toAMPM(SAT_END)}) 예약 불가</div>}
                                {adminBookIsWedBlocked&&<div style={{background:"#fff7ed",border:"1.5px solid #fb923c55",borderRadius:c.radiusSm,padding:"8px 12px",fontSize:12,color:"#c2410c"}}>🙏 수요예배 시간({toAMPM(WED_START)}–{toAMPM(WED_END)}) 예약 불가</div>}
                                ...
                                {adminBookForm.startTime&&adminBookForm.endTime&&adminBookForm.startTime>=adminBookForm.endTime&&<div style={{fontSize:12,color:c.danger,background:c.dangerBg,borderRadius:c.radiusSm,padding:"6px 10px"}}>종료 시간은 시작 시간보다 늦어야 합니다</div>}
