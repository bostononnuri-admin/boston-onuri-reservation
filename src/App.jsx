...
export default function App() {
  ...
  const isTimeInWorship  = t => t >= WORSHIP_START && t < WORSHIP_END;
  ...
  // 선택한 시작~종료 시간이 예배 시간과 겹치는지 확인
  const overlapsWorship = isWorshipBlocked && form.startTime && form.endTime &&
    form.startTime < WORSHIP_END && form.endTime > WORSHIP_START;

  const canSubmit = form.name && isPhoneValid(form.phone) && form.purpose &&
    form.startTime && form.endTime && form.startTime < form.endTime && !overlapsWorship;

  async function handleSubmit() {
    if (!canSubmit) return;
    const space = SPACES.find(s => s.id === selSpace.id);
    const newRes = { id: String(Date.now()), spaceId: selSpace.id, spaceName: space?.name||"", date: selDate, ...form, recurring: false };
  ...
  async function handleRecurSubmit() {
    ...
    while (cur <= end) {
      if (recurType === "daily" || String(cur.getDay()) === recurType) {
        rows.push({ id: String(Date.now() + Math.random()), spaceId: parseInt(spaceId), spaceName: space?.name||"", date: fmtDate(cur), startTime, endTime, name, phone, team, purpose, note, password:"", recurring:true });
      }
      cur.setDate(cur.getDate() + 1);
    }
    setReservations(p => [...p, ...rows]);
    setRecurDone(true);
    await gasRequest("addBatch", { reservations: rows });
  }
              ...
              <div style={{display:"flex",flexDirection:"column",gap:12}}>
                ...
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
                  {[["startTime","시작 시간"],["endTime","종료 시간"]].map(([k,lbl])=>(
                    <div key={k}><div style={{fontSize:13,fontWeight:600,color:c.sub,marginBottom:5}}>{lbl}</div><select value={editForm[k]} onChange={e=>setEditForm(f=>({...f,[k]:e.target.value}))} style={IS}>{TIME_SLOTS.map(t=><option key={t} value={t}>{toAMPM(t)}</option>)}</select></div>
                  ))}
                </div>
        ...
        <div style={{maxWidth:900,margin:"0 auto",display:"flex",alignItems:"center",justifyContent:"space-between",height:58}}>
          ...
          <div style={{display:"flex",gap:6}}>
            {[["home","예약 신청"],["admin","관리자"]].map(([v,lbl])=>(
              <button key={v} onClick={()=>setView(v)} style={{background:view===v?c.chipBg:"transparent",border:`1.5px solid ${view===v?c.primary:c.border}`,color:view===v?c.primary:c.sub,borderRadius:c.radiusSm,padding:"7px 12px",cursor:"pointer",fontSize:13,fontFamily:"inherit",fontWeight:view===v?700:400}}>{lbl}</button>
            ))}
          </div>
        </div>
      </header>
                      ...
                      {homeSelRes.length===0
                        ...
                        :<div style={{display:"flex",flexDirection:"column",gap:8}}>
                          {homeSelRes.map(r=>{
                            ...
                            return (
                              <div key={r.id} style={{display:"grid",gridTemplateColumns:"auto 1fr",gap:12,alignItems:"center",borderRadius:c.radiusSm,padding:"10px 14px",background:c.chipBg,border:`1px solid ${c.primary}22`}}>
                ...
                :<div style={{display:"flex",flexDirection:"column",gap:14}}>
                  ...
                  {isWorshipBlocked&&(
                    <div style={{background:"#fff7ed",border:"1.5px solid #fb923c55",borderRadius:c.radiusSm,padding:"10px 14px",fontSize:13,color:"#c2410c"}}>
                      ...
                      매주 일요일 8:00 AM – 12:30 PM 은 이 공간을 예약할 수 없습니다.
                    </div>
                  )}
                    ...
                    {[["startTime","시작 시간"],["endTime","종료 시간"]].map(([key,lbl])=>(
                      <FL key={key} label={lbl} required>
                        <select value={form[key]} onChange={e=>setForm(f=>({...f,[key]:e.target.value}))} style={{...IS,color:form[key]?c.text:c.sub}}>
                          ...
                          {TIME_SLOTS.map(t=>{
                            const busy = key==="startTime"
                              ? isSlotBusy(t)
                              : (isWorshipBlocked && t > WORSHIP_START && t <= WORSHIP_END
                                  ? (form.startTime < WORSHIP_END)
                                  : false) || dateRes.some(r => form.startTime >= r.startTime && t > r.startTime && form.startTime < r.endTime && t <= r.endTime);
                            const worshipBlock = isWorshipBlocked && isTimeInWorship(t);
                            return (
                              <option key={t} value={t} disabled={busy||worshipBlock}>
                                {toAMPM(t)}{(busy||worshipBlock)?" ●":""}
                              </option>
                            );
                          })}
                        </select>
                  ...
                  {overlapsWorship&&<div style={{fontSize:13,color:"#c2410c",background:"#fff7ed",borderRadius:c.radiusSm,padding:"8px 12px"}}>⛪ 선택한 시간이 주일예배 시간(8:00 AM – 12:30 PM)과 겹칩니다</div>}
