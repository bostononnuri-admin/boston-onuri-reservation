...
export default function App() {
  ...
  async function handleRecurSubmit() {
    ...
    setRecurDone(true);
    for (const r of rows) { await gasRequest("add", { reservation: r }); }
  }
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(130px,1fr))",gap:10}}>
              {SPACES.map(sp=>(
                <div key={sp.id} onClick={()=>{setSelSpace(sp);setView("book");}}
                  style={{background:c.card,border:`1.5px solid ${c.border}`,borderRadius:c.radius,padding:"18px 10px",cursor:"pointer",transition:"all .15s",boxShadow:c.shadow,textAlign:"center"}}
                  ...
                  <div style={{fontSize:30,marginBottom:8}}>{sp.icon}</div>
                  <div style={{fontSize:14,fontWeight:700,marginBottom:OVERLAP_ALLOWED.includes(sp.id)?3:0}}>{sp.name}</div>
                  {OVERLAP_ALLOWED.includes(sp.id)&&<div style={{fontSize:10,color:c.primary,fontWeight:600}}>중복예약 가능</div>}
                </div>
              ))}
            </div>
            <div style={{marginTop:44,display:"flex",flexDirection:"column",alignItems:"center",gap:8,opacity:0.5}}>
              <img src="/logo.jpg" alt="로고" style={{width:60,height:60,borderRadius:12,objectFit:"cover"}}/>
              <div style={{fontSize:12,color:c.sub,letterSpacing:1}}>보스턴 온누리교회</div>
            </div>
          </div>
        )}
